...
class _TagInputState extends State<TagInput> {
  ...
  void initState() {
    super.initState();
    _suggestions = widget.suggestedTags ?? ['doctor', 'patient', 'appointment', 'medication', 'treatment'];
  }
  ...
  void _removeTag(String tag) {
    ...
    if (widget.requiredTags != null && widget.requiredTags!.contains(tag)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('The tag "$tag" is required and cannot be removed')),
      );
      return;
    }
      ...
      children: [
        ...
        Wrap(
          ...
          children: widget.tags.map((tag) {
            final bool isRequired = widget.requiredTags != null && 
                                   widget.requiredTags!.contains(tag);
            return Chip(
              label: Text(tag),
              deleteIcon: isRequired ? null : const Icon(Icons.close, size: 18),
