...
class _AISchedulingAssistantState extends State<AISchedulingAssistant> {
  ...
  Future<void> _processQuery() async {
    ...
    try {
      ...
      final availableDoctors = await _findMatchingDoctors(recommendedSpecialties);
  ...
  Future<void> _generateRecommendations(List<User> doctors, String query) async {
    if (doctors.isEmpty) {
      setState(() {
        _errorMessage = 'No doctors found for your symptoms. Please try a different description.';
      });
      return;
    }
    ...
    for (var doctor in doctors) {
      try {
        ...
        final bookedSlots = await _getBookedTimeSlots(doctor.id, startOfDay, endOfWeek);
        ...
        if (nextAvailableDate != null && availableTimeSlot != null) {
          ...
          final specialtyMatchScore = _calculateSpecialtyMatchScore(doctor, query);
  ...
  List<String> _getAvailableSlotsForDate(DateTime date, Map<DateTime, List<String>> bookedSlots) {
    // Standard time slots
    final allTimeSlots = [
    ...
    if (bookedSlots.containsKey(normalizedDate)) {
      return allTimeSlots.where((slot) => !bookedSlots[normalizedDate]!.contains(slot)).toList();
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('AI Scheduling Assistant'),
      ),
                      ...
                      const Center(
                        child: Column(
                          children: [
                            ...
                            SizedBox(height: 16),
                            Text('Analyzing your symptoms and finding the best doctor...'),
                          ],
                        ),
                      ),
                    ],
                      ...
                      Expanded(
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final timeSlot = recommendation['timeSlot'] as String;
                            final urgency = recommendation['urgency'] as double;
                            
                            final isSelected = _selectedRecommendation == recommendation;
      ...
      floatingActionButton: _selectedRecommendation != null
