...
class EmergencyService {
  ...
  Future<void> _notifyEmergencyContacts(EmergencyAlert alert) async {
    try {
      ...
      final contactsToNotify =
          contacts.where((c) => c.notifyOnEmergency).toList();

      if (contactsToNotify.isEmpty) return;

      // Get user info for more detailed alert
      final userDoc =
          await _firestore.collection('users').doc(alert.userId).get();
      final userData = userDoc.data();

      // Craft emergency message
      String emergencyType = alert.type.toString().split('.').last;
      String mapLink =
          'https://www.google.com/maps/search/?api=1&query=${alert.location.latitude},${alert.location.longitude}';

      String message =
          'EMERGENCY ALERT: ${alert.userName} has triggered a $emergencyType emergency. ';
      ...
      List<String> phoneNumbers =
          contactsToNotify.map((c) => c.phoneNumber).toList();
      // await SmsMms(message: message, recipients: phoneNumbers);
      await SmsMms.send(recipients: phoneNumbers, message: message);

      // Update the alert with notified contacts
      List<String> notifiedContactIds =
          contactsToNotify.map((c) => c.id).toList();
      await _firestore.collection('emergency_alerts').doc(alert.id).update({
